...
myGame model = 
    [ 
    {-Frame-}
        rect 300 300 |> filled (rgb 255 255 180)
    {-Cake-}
        ...
        , oval 7 10 |> filled orange |> move(0,57) |> scale (sin(3*model.time))
        ...
        , circle 5 |> filled green |> move (35,-15)
    {-Butterfly-}
        , oval 50 27 |> filled pink |> move (80,80) |> rotate (degrees 210) |> rotate (degrees <| 12.5 * sin(2*model.time))
        , oval 50 27 |> filled pink |> move (80,80) |> rotate (degrees 135) |> rotate (degrees <| 12.5 * cos(2*model.time))
        , oval 50 7 |> filled green |> move (80,80) |> rotate (degrees 75) |> rotate (degrees <| 12.5 * sin(2*model.time))
        , oval 50 27 |> filled pink |> move (-80,-80) |> rotate (degrees 210) |> rotate (degrees <| 12.5 * sin(2*model.time))
        , oval 50 27 |> filled pink |> move (-80,-80) |> rotate (degrees 135) |> rotate (degrees <| 12.5 * cos(2*model.time))
        , oval 50 7 |> filled green |> move (-80,-80) |> rotate (degrees 75) |> rotate (degrees <| 12.5 * sin(2*model.time))
     {-Text-}

...
cooler model = group [rect 8 10 |> filled blank |> addOutline (solid 0.5) black
                     , rect 20 2 |> filled blank |> addOutline (solid 0.5) black |> move (10,0)
                     , rect 8 0.5 |> outlined (dotted 1) (myColor (case model.selected of Freezer -> model.fcolor
                                                                                          Refrigerator -> model.rcolor
                                                                                          _ -> Blue1)) |> move (abs(10*sin(model.time)),0)
                     , circle 4 |> filled blue
                     ]

indr model = group [ roundedRect 5 50 5 |> filled blank |> addOutline (solid 1) white
                  , roundedRect 5 (case model.rtemp of 0 -> 0
                                                       1 -> 10
                                                       2 -> 20
                                                       3 -> 30
                                                       4 -> 40
                                                       _ -> 50) 5 |> filled (myColor model.rcolor) |> addOutline (solid 1) white
                  , circle 5 |> filled (myColor model.rcolor) |> addOutline (solid 1) white |> move (0,-20)
                  ]

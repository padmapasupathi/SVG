...
init = { time = 0 , myTractor = (50,0) , selected = SelectNone , pos = Unload , item = None , items = "Apple" , board = 1 , shp = 0 , shp1 = 0 , x = 0 , clouds = clouds1 , score = 0 , bgPumpkin = bgPumpkin1 , bgDog = bgDog1 , bgHouse = bgHouse1 }
...
type Msg = Tick Float GetKeyState | ClickLoad | ClickApple | ClickTomato | ClickPumpkin | ClickBoard | ClickItem

type Position = Load | Unload

type Veg = Apple | Tomato | Pumpkin | None
    ...
    case msg of
        Tick t (_,(nx,ny),_) ->
                       let
                         ...
                         newPosX = if oldPosX > 150 then
                                         150
                                      else if oldPosX < -200 then
                                         -200
                                      else oldPosX + 0.5*nx-- Increase the speed of navigation
                         {-(oldPos1X,oldPos1Y) = model.bgHouse
                         nPos1X = if oldPos1X > 215 then 215 else if oldPos1X < -215 then 215 else oldPos1X - nx
                         -}
                       in
                         { model | time = t
                         ...
                         , x = model.x + nx
                         , clouds = List.map (\(x,y) -> if x > 115 then (-115,15 * sin (100*model.time)+30) else if x < -115 then (115,15 * sin (30*model.time)+30) else (x - nx,y)) model.clouds
                         , bgHouse = List.map (\(x,y) -> if x > 400 then (400,y) else if x < -25 then (-25,y) else (x - nx,y)) model.bgHouse
                         , bgPumpkin = List.map (\(x,y) -> if x > -200 then (-200,y) else if x < -815 then (-815,y) else (x - nx,y)) model.bgPumpkin
                         , bgDog = List.map (\(x,y) -> if x > -200 then (-200,y) else if x < -515 then (-515,y) else (x - nx,y)) model.bgDog
        ...
        ClickBoard -> { model | board = round (abs(10*cos(model.time*100)))
                           , items = case round (abs(3*sin(model.time*100))) of
                                       1 -> "Apple"
                                       2 -> "Tomato"
                                       _ -> "Pumpkin"
                           , shp1 = model.board
                           , shp = 0
                           , score = if model.pos == Unload then
                                        model.score + 1
                                   else model.score
                                   }

        ClickApple -> { model | selected = SelectApple }
        ClickTomato -> { model | selected = SelectTomato }
...
view model =
    collage 1250 900
        ...
        --, apple |> scale 1 |> move (-520,-25) |> notifyTap ClickApple |> notifyTap ClickLoad
        --, tomato |> scale 1 |> move (-455, -20) |> notifyTap ClickTomato |> notifyTap ClickLoad
        -- , pumpkin |> scale 1 |> move (-400,-20) |> notifyTap ClickPumpkin |> notifyTap ClickLoad
        , unicorn  |> scale 1 |> scale (magicFun model)  |> move (0,300)
        -- , mushroomHouse |> scale 2 |> move (550,50)
        , reqBoard model |> move (200, 100)
        , scoreBoard model |> move (100,200)]


showBg1 bgHouse model = group (List.map (\(x,y) -> group [ model.mushroomHouse  |> move (x,y)]) bgHouse )
...
background model = group [
   ...
   , showClouds model.clouds |> scale 4 |> move (0,150)
            ...
            , ( group <| List.map (\id -> loadUnload model.item |> scale 0.75 |> move (drawShape id)) (List.range 1 model.shp1) ) -- |> notifyTap ClickItem
            , showBg2 model.bgPumpkin |> move (-150,0) |> notifyTap ClickPumpkin |> notifyTap ClickLoad
            , showBg3 model.bgDog |> move (-150,-50)
            ]
--clouds1 = [(-60,30),(-20,10),(-5,40),(50,50),(30,10),(-80,7),(75,20)]
...
magicFun model = if (remainderBy 10 model.score == 0) && (model.score /= 0) then
                    2
                 else
                    1

happyDog model = if (remainderBy 10 model.score == 0) && (model.score /= 0) then
                    50*sin (1*model.time)
                 else
                    -100
...
reqBoard model = group [ rect 120 30 |> filled white |> addOutline (solid 1) black |> move (50,0)
                 , text (Debug.toString model.board ++ " "++ Debug.toString model.items ++ " reqired!") |> size 12 |> filled black |> notifyTap ClickBoard
    ] |> notifyTap ClickBoard

scoreBoard model = group [rect 120 30 |> filled white |> addOutline (solid 1) black |> move (50,0)
                 , text ("Score : " ++ Debug.toString model.score) |> size 12 |> filled black ]
...
tomato  = group [ oval 50 43 |> filled darkOrange |> addOutline (solid 1) black
...
apple = group [ curve (30,-7) [Pull (16,-18) (3,0), Pull (-10,15) (10,30), Pull (17,36) (29,27) ] |> filled red |> addOutline (solid 1) black
